  ...
  try {
    ...
    const asEntries = parsed
      .map((x): ScrapEntry | null => {
        ...
        if (typeof candidate.stockLength === "number" && typeof candidate.quantity === "number") {
          return {
            materialType: "board",
...
function App() {
  ...
  const [scrapInventory, setScrapInventoryState] = useState<ScrapEntry[]>(() => loadScrapFromStorage())
  ...
  const setScrapInventory = (next: ScrapEntry[] | ((prev: ScrapEntry[]) => ScrapEntry[])) => {
  ...
  const hasCuts = groups.some((g) => g.materialType === "board" && g.cuts.length > 0)
  const everyGroupHasValidBoardSpec = groups.every(
    (g) => g.materialType !== "board" || (g.boardSpecId && STOCK_PROFILES.some((p) => p.id === g.boardSpecId))
  )
  const canGenerate = hasCuts && everyGroupHasValidBoardSpec
  const generationErrors: string[] = []
  if (!hasCuts) generationErrors.push("Add at least one cut to generate a plan.")
  if (!everyGroupHasValidBoardSpec) generationErrors.push("Select a board spec for every material group.")
  ...
  useEffect(() => {
    if (readStoredTheme() !== null || typeof window === "undefined" || typeof window.matchMedia !== "function") {
      return
    }
  ...
  const handleGenerate = () => {
    ...
    const nonBoardScrap = scrapInventory.filter((s) => s.materialType !== "board")
    ...
    const remainingBoardScrap = boardScrap
      ...
      .filter((entry): entry is Extract<ScrapEntry, { materialType: "board" }> => entry !== null)
    ...
    const usedCount = [...usedFromScrap.values()].reduce((sum, n) => sum + n, 0)
    if (usedCount === 0) {
    ...
    <div className="min-h-screen bg-slate-100 dark:bg-slate-900 py-6 px-4 sm:p-6 lg:py-8 lg:px-8 print:bg-white print:py-0 print:px-0 overflow-x-hidden">
      <div className="w-full mx-auto transition-all duration-200 print:max-w-none print:safe-inset print:grid-cols-1 lg:max-w-[1920px] lg:grid lg:grid-cols-[minmax(0,1fr)_minmax(0,1.2fr)] lg:gap-x-10 lg:items-start">
        ...
        <div className="mt-8 lg:mt-0 print:mt-0 space-y-4">
          <section className="print:hidden rounded-xl bg-slate-50/80 dark:bg-slate-800/40 p-4 lg:p-5 space-y-3">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <label className="inline-flex items-center gap-2 cursor-pointer">
                ...
                />
                <span className="text-sm text-slate-700 dark:text-slate-300">Use scrap for this run</span>
              </label>
            </div>
            <p className="text-sm text-slate-600 dark:text-slate-400">
              Assumptions: {DEFAULT_KERF_INCHES}" kerf (1/8″) per cut, max board length preference defaults to {DEFAULT_MAX_BOARD_LENGTH_INCHES / 12} ft.
            </p>
          ...
          {projectResult ? (
            <UnifiedResultsView result={projectResult} scrapNote={lastRunScrapNote} />
          ) : (
            <div className="print:hidden px-1 py-2">
              <p className="text-slate-500 dark:text-slate-400 text-sm">
                Your shopping list and cut diagrams appear here after you generate.
              </p>
            </div>
          )}
        </div>
      ...
      <footer className="print:hidden mt-8 text-center text-xs text-slate-500 dark:text-slate-400">
        <p>
          ...
          {" "}· Open source projects made with love (but really for clout).
        </p>
      </footer>
    </div>
  )
